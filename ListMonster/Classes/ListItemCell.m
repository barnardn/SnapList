...
- (void)setNeedsLayout
{
    ...
    if ([parentView isEditing])
    {
        ...
        contentFrame.origin.x = contentMargin +  35.0f;
    ...
    else
    {
        ...
        CGRect contentFrame = [self contentView].frame;
        contentFrame.origin.x = contentMargin + 25.0f;  //CELL_CONTENT_MARGIN;
        [self contentView].frame = contentFrame;
    }
...
- (void)adjustNameLabel:(UILabel *)nameLabel isEditing:(BOOL)editing
{
    ...
    CGFloat contentMargin = CELL_CONTENT_MARGIN;
    CGFloat lblStart = [self contentView].frame.origin.x;
